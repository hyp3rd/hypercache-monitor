      ...
      toast[failed === 0 ? "success" : "warning"](
        `Stored ${stored.toLocaleString()} of ${out.results.length.toLocaleString()}`,
        {
          description:
            failed === 0
              ? "All items written."
              : `${failed} item(s) failed — see results table.`,
        },
      );
    } catch (e) {
      ...
      <div className="border-border/50 bg-card/40 space-y-4 rounded-lg border p-4">
        ...
        <div className="flex items-center justify-between gap-3">
          <p className="text-muted-foreground text-xs">
            Chunked at 1,000 items per request. Per-item failures don&apos;t
            halt the batch.
          </p>
          <Button
            onClick={run}
    ...
    cell: ({ row }) =>
      row.original.bytes !== undefined ? (
        <span className="text-muted-foreground font-mono text-xs">
          {row.original.bytes.toLocaleString()}
        </span>
      ) : (
