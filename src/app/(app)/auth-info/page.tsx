...
export const metadata: Metadata = {
  title: "Auth posture",
  description: "Session identity, granted scopes, and the cache's advertised security schemes.",
};
...
export default async function AuthInfoPage() {
  ...
  try {
    ...
    specError = err instanceof SpecFetchError ? err.message : "spec fetch failed";
  }
  ...
  return (
    <div className="space-y-6">
      <header>
        ...
        <h1 className="mt-1 text-3xl font-semibold tracking-tight">Auth posture</h1>
        <p className="text-muted-foreground mt-2 max-w-2xl text-sm">
          Session identity, granted scopes, and the cache&apos;s advertised security schemes for{" "}
          <span className="text-foreground font-mono font-semibold">{auth.clusterId}</span>. Bearer tokens are
          never displayed.
        </p>
      </header>

      <IdentityCard clusterId={auth.clusterId} identity={auth.session.identity} />
      <ScopesCard granted={auth.session.scopes} />
      {spec ? (
        <SchemeCard schemes={spec.components?.securitySchemes} specVersion={spec.info.version} />
      ) : (
  ...
  return (
    <div role="alert" className="border-border/50 bg-card/60 rounded-lg border p-4 text-sm backdrop-blur">
      ...
      <p className="text-muted-foreground mt-1 text-xs">
        ...
        <span className="text-destructive font-mono">{message}</span>. The Identity and Scopes cards above
        still reflect your live session; only the cache-side security-scheme card is missing.
      </p>
    </div>
  );
}
