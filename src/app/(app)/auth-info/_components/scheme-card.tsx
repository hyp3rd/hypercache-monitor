    ...
    <Card className="border-border/50 bg-card/60 backdrop-blur">
      <CardHeader className="flex-row items-center gap-3 space-y-0">
        ...
        <div>
          ...
          <CardDescription>
            From the cache&apos;s OpenAPI spec — what it expects on the wire.
          </CardDescription>
        </div>
      </CardHeader>
      <CardContent className="space-y-4">
        <p className="text-muted-foreground text-xs">
          Spec version{" "}
          <span className="text-foreground font-mono">{specVersion}</span>
        </p>
        {entries.length === 0 ? (
          <p
            role="alert"
          ...
          >
            ...
            />
            The spec defines no security schemes. Verify the cache is configured
            with HYPERCACHE_AUTH_TOKEN — an empty schemes block is unusual on a
            production cluster.
          </p>
        ) : (
          <ul
          ...
          >
            {entries.map(([name, scheme]) => (
              <li
                key={name}
                className="border-border/50 bg-card/50 rounded-lg border p-3"
              >
