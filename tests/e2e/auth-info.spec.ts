...
test.describe("auth posture", () => {
  test("renders identity, scopes, and security scheme from live state", async ({ page }) => {
    await login(page);
    await page.goto("/auth-info");

    await expect(page.getByRole("heading", { name: "Auth posture", level: 1 })).toBeVisible();

    // Identity card
    await expect(page.getByText("Session identity", { exact: true })).toBeVisible();
    await expect(page.getByText("Active cluster", { exact: true })).toBeVisible();

    // Scopes card — login probe path grants read+write+admin in
    // the stub session bootstrap (same shape every test uses).
    await expect(page.getByText("Granted scopes", { exact: true })).toBeVisible();
    ...
    await expect(page.getByText("Server auth scheme", { exact: true })).toBeVisible();
    await expect(page.getByText("bearerAuth", { exact: true })).toBeVisible();
    // type · scheme pill on the bearerAuth row
    await expect(page.getByText("http · bearer", { exact: true })).toBeVisible();
  });
